...
1. Select the folder containing the experiment's images (e.g. `images/$EXP/images/250808_101736_AONPPS_Protocol/`). **Do not rename folders or files after this step** — CellProfiler will lose the reference.
...
|---|---|---|---|
| `01_samples_retrieval.py` | CellProfiler CSV/SQLite | `single_cell_profiles.parquet` | reads and consolidates profiles, structural sanity checks (SC-01→SC-05) |
| `02_aggregate_normalize_featureselect.py` | `single_cell_profiles.parquet` | `per_well_features_selected.parquet` | annotates with the platemap, aggregates to well level, normalizes (`mad_robustize`), selects features (SC-06→SC-10) |
| `03_phenotypic_profiling.py` | `per_well_features_selected.parquet` | PCA/UMAP/LDA figures and models | PCA, UMAP, LDA with leave-one-plate-out CV, Cohen's d, KMeans, uncorrected-vs-Harmony comparison (SC-11→SC-13) |
| `04_phenotypic_fingerprints.py` | `per_well_features_selected.parquet` | fingerprints by feature taxonomy | feature taxonomy, per-condition Cohen's d, heatmaps/radar plots — dose axis optional |
| `05_quality_metrics.py` | `per_well_features_selected.parquet` | quality metrics + decision | Percent Replicating, mAP, batch assessment, dose-response, **Go/No-Go** decision (SC-18→SC-22) |
...
| `07_recovery_axis_analysis.py` (optional) | `per_well_features_selected.parquet` | recovery axis | geometric axis between two reference states — only applies to experimental designs with that structure; skips itself automatically if not configured |
